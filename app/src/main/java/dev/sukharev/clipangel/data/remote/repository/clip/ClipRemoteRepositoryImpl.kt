...
import dev.sukharev.clipangel.utils.AESCrypt
...
class ClipRemoteRepositoryImpl(private var firebaseDb: FirebaseDatabase,
    ...
    override fun get(channelId: String): Flow<Result<Clip>> = callbackFlow {
        firebaseDb.getReference(channelId).apply { keepSynced(true) }.child("data").get().addOnSuccessListener { snapshot ->
            GlobalScope.launch {
                try {
    ...
    private fun decrypt(data: String, secret: String) = String(AESCrypt.decrypt(
            Base64.getDecoder().decode("WEGWfAm13eERkKMaLVeKHw=="),
